...
(() => {
 ...
 if(p.chartFeed){const charts=node('div',null,'connection-symbols');charts.append(node('p','Public charts · independent of account login'));for(const asset of ['BTC','ETH','SOL']){const b=node('button',asset+' '+(p.id==='hyperliquid'?'perpetual':'/ USD'),'primary');b.onclick=()=>{if(state.charts.length>=8){notify('This desk has eight charts. Remove one before adding another.');return;}const c=makeChart({asset,feed:p.chartFeed,contract:p.id==='hyperliquid'?'perp':'spot',w:1,h:1});state.charts.push(c);dialog.close();render();arrange();saveSoon();};charts.append(b);}target.append(charts);}
